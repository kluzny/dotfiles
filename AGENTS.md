...
- **`bin/`** — personal CLI utilities (`dirty.sh`, `ramchop.sh`, `when.sh`/`when.rb`). `scripts/install_bins.sh` symlinks everything in `bin/` into `~/.local/bin`; nothing else installs them. `.tmux.conf`'s status bar shells out to `~/.local/bin/ramchop` directly, so that symlink has to exist for the tmux status bar to render correctly.
...
- **`Makefile`** — `make help` lists targets; `make lint` runs this repo's own syntax checks (shellcheck globbed over `bin/*.sh`/`scripts/*.sh`, `zsh -n`, `ruby -c`, `luac -p`, `jq`, a TOML parse). See "Things worth knowing before editing" for what's covered.
